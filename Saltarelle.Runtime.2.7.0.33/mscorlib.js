...
var ss_ILocaleFormattable = function ILocaleFormattable$() { };
...
ss.format = function ss$format(obj, fmt, useLocale) {
...
var ss_OperationCanceledException = function OperationCanceledException$(message, token, innerException) {
...
var ss_CancellationTokenRegistration = function CancellationTokenRegistration(cts, o) {
	this._cts = cts;
	this._o = o;
};
...
ss.initClass(ss_CancellationTokenRegistration, ss, {
	dispose: function CancellationTokenRegistration$dispose() {
		if (this._cts) {
			this._cts._deregister(this._o);
			this._cts = this._o = null;
		}
	},
	equalsT: function CancellationTokenRegistration$equalsT(o) {
		return this === o;
	}
}, Object, [ ss_IDisposable, ss_IEquatable ]);
ss_CancellationTokenRegistration.__class = false;
ss_CancellationTokenRegistration.getDefaultValue = function CancellationTokenRegistration$default() { return new ss_CancellationTokenRegistration(); };

///////////////////////////////////////////////////////////////////////////////
// CancellationTokenSource

var ss_CancellationTokenSource = function CancellationTokenSource$(delay) {
...
ss_CancellationTokenSource.createLinked = function CancellationTokenSource$createLinked() {
...
ss_CancellationTokenSource.__typeName = 'ss.CancellationTokenSource';
ss.CancellationTokenSource = ss_CancellationTokenSource;
ss.initClass(ss_CancellationTokenSource, ss, {
	cancel: function TaskCompletionSource$cancel(throwFirst) {
	...
	cancelAfter: function TaskCompletionSource$cancelAfter(delay) {
		if (this.isCancellationRequested)
			return;
		if (this._timeout)
	...
	_register: function TaskCompletionSource$_register(f, s) {
		if (this.isCancellationRequested) {
			f(s);
	...
	_deregister: function TaskCompletionSource$_deregister(o) {
		var ix = this._handlers.indexOf(o);
		if (ix >= 0)
			this._handlers.splice(ix, 1);
	},
	dispose: function TaskCompletionSource$dispose(delay) {
		this._clean();
	},
	_clean: function TaskCompletionSource$_clean(delay) {
		if (this._timeout)
			clearTimeout(this._timeout);
		this._timeout = null;
		this._handlers = [];
...
}, Object, [ss_IDisposable]);

///////////////////////////////////////////////////////////////////////////////
// CancellationToken

var ss_CancellationToken = function CancellationToken(source) {
...
ss.initClass(ss_CancellationToken, ss, {
	get_canBeCanceled: function CancellationToken$get_canBeCanceled() {
		return !this._source._uncancellable;
	},
	get_isCancellationRequested: function CancellationToken$get_isCancellationRequested() {
		return this._source.isCancellationRequested;
	},
	throwIfCancellationRequested: function CancellationToken$throwIfCancellationRequested() {
		if (this._source.isCancellationRequested)
			throw new ss_OperationCanceledException(this);
	},
	register: function CancellationToken$register(cb, s) {
		return this._source._register(cb, s);
	}
});
ss_CancellationToken.__class = false;
ss_CancellationToken.getDefaultValue = function CancellationToken$default() { return new ss_CancellationToken(); };
...
var ss_TaskCanceledException = function TaskCanceledException$(message, task, innerException) {
